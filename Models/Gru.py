...
class EfficientGRUModel(nn.Module):
    ...
    def __init__(self, input_size = 768, hidden_size = 512, output_size = 6, number_layers = 2):
        super().__init__()
...
class GRUDecoder(nn.Module):
    ...
    def __init__(self, hidden_size = 512, vocab_size = 32, number_layers = 5):
        ...
        self.fc_out1 = nn.Linear(3*hidden_size, hidden_size//2) 
        self.gelu = nn.GELU(approximate='tanh') 
        self.fc_out2 = nn.Linear(hidden_size//2, vocab_size) 
    ...
    def forward(self, enc_output, shot, hidden):
        ...
        pre_logits = self.fc_out1(self.ln_out_dec(pre_logits)) 
        pre_logits = self.gelu(pre_logits) 
        logits = self.fc_out2(pre_logits) # (1, vocab_size) 
    ...
    def forward(self, x, y=None, h=None):
        ...
        if y is not None:
            for i in range(len(y)-1):
                # teacher forcing
                # teacher_forcing_probability = np.exp(-0.0002*step) if step is not None else 1.0
                # teacher_forcing_probability = torch.tensor(teacher_forcing_probability, device=y.device)
                # teacher_force = torch.rand((), device=y.device) < teacher_forcing_probability
                # token = torch.where(teacher_force, y[i], current_token)
                logit, h = self.decoder(enc_output, y[i], h) # (vocab_size,), (Layers, 1, hidden_size)
                # current_token = logit.argmax()
